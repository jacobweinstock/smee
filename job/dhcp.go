...
func (j Job) configureDHCP(ctx context.Context, rep, req *dhcp4.Packet) bool {
	...
	if dhcp.SetupPXE(ctx, j.Logger, rep, req) {
		...
		arch := getArch(j.hardware, req.GetCHAddr().String())
		if dhcp.Arch(req) != arch {
			span.AddEvent(fmt.Sprintf("arch mismatch: got %q and expected %q", dhcp.Arch(req), arch))
			j.Logger.Info("arch mismatch, using dhcp", "dhcpArch", dhcp.Arch(req), "jobArch", arch)
		}

		isUEFIReq := dhcp.IsUEFI(req)
		if isUEFIReq != isUEFI(j.hardware, req.GetCHAddr().String()) {
			j.Logger.Info("uefi mismatch, using dhcp", "dhcpUEFI", isUEFIReq, "jobUEFI", isUEFI(j.hardware, req.GetCHAddr().String()))
		}
		...
		j.setPXEFilename(rep, isTinkerbellIPXE, isARM, isUEFIReq, dhcp.IsHTTPClient(req))
	} else {
...
func (j Job) setPXEFilename(rep *dhcp4.Packet, isTinkerbellIPXE, isARM, isUEFI, isHTTPClient bool) {
	if j.metadataState() == "in_use" {
		if j.metadataInstance().ID == "" {
		...
		if j.metadataInstance().State != "active" {
			j.Logger.Info("device should NOT be trying to PXE boot", "hardware.state", j.metadataState(), "instance.state", j.metadataInstance().State)
		...
		if !j.AllowPXE() && j.getOperatingSystem().OsSlug != "custom_ipxe" {
			// if !j.AllowPXE() && j.hardware.OperatingSystem().OsSlug != "custom_ipxe" {
			j.Logger.Info("device should NOT be trying to PXE boot", "hardware.state", j.metadataState(), "allow_pxe", j.AllowPXE(), "os", j.getOperatingSystem().OsSlug)
	...
	case !j.AllowPXE():
		...
		os := j.getOperatingSystem()
		j.Logger.Info("info", "instance.state", j.metadataState(), "os_slug", os.Slug, "os_distro", os.Distro, "os_version", os.Version)
...
func (j *Job) VLANID() string {
	if j.ifDHCP != nil && j.ifDHCP.DHCP.VLANID != "" {
		return j.ifDHCP.DHCP.VLANID
	}
